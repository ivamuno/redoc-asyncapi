...
export function TryItOutExample({ example, mimeType }: TryItOutExampleProps) {
  if (example.value === undefined && example.externalValueUrl) {
    return <ExternalExample example={example} mimeType={mimeType} />;
  } else {
    return <TryItOutExampleValue value={example.value} mimeType={mimeType} />;
  }
}

export function ExternalExample({ example, mimeType }: TryItOutExampleProps) {
  ...
  return <TryItOutExampleValue value={value} mimeType={mimeType} />;
}
